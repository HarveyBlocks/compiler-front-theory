...
> 1: [ERROR] Unknown lifecycle phase ".git". You must specify a valid lifecycle phase or a goal in the format <plugin-prefix>:<goal> or <plugin-group-id>:<plugin-artifact-id>[:<plugin-version>]:<goal>. Available lifecycle phases are: validate, initialize, generate-sources, process-sources, generate-resources, process-resources, compile, process-classes, generate-test-sources, process-test-sources, generate-test-resources, process-test-resources, test-compile, process-test-classes, test, prepare-package, package, pre-integration-test, integration-test, post-integration-test, verify, install, deploy, pre-clean, clean, post-clean, pre-site, site, post-site, site-deploy. -> [Help 1]
  ...
  7: [ERROR] [Help 1] http://cwiki.apache.org/confluence/display/MAVEN/LifecyclePhaseNotFoundException
  8: 
```
...
```
> 1: ﻿mvn : WARNING: A restricted method in java.lang.System has been called
  2: At line:2 char:75
  3: + ... ntic.debugFunction=true'; mvn -Dtest=Text1DebugTest test 2>&1 | Tee-O ...
  4: +                               ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  5:     + CategoryInfo          : NotSpecified: (WARNING: A rest...has been called:String) [], RemoteException
  6:     + FullyQualifiedErrorId : NativeCommandError
  7:  
  8: WARNING: java.lang.System::load has been called by org.fusesource.jansi.internal.JansiLoader in an unnamed module (file
  9: :/D:/mvn/apache-maven-3.8.8/lib/jansi-2.4.0.jar)
  10: WARNING: Use --enable-native-access=ALL-UNNAMED to avoid a warning for callers in this module
  11: WARNING: Restricted methods will be blocked in a future release unless native access is enabled
```
...
```java
  74:             .when(TypeRule.NUMERIC_BINARY, ProgramSemanticTag.DIVIDE)
  75:             .when(TypeRule.ASSIGNMENT, ProgramSemanticTag.ASSIGNMENT)
  76:             .when(TypeRule.VOID_FUNCTION_HEAD, ProgramSemanticTag.FUNCTION, ProgramSemanticTag.HEAD)
  77:             ;
  78: 
  79:     @Override
  80:     public void onReduce(ShiftReduceSemanticContext context, SimpleGrammarProduction production) {
  81:         HeadNode head = currentReducedHead(context);
  82:         if (head.getSymbol().isDefine()) {
  83:             String defineName = head.getSymbol().toDefine().getName();
> 84:             if ("loc".equals(defineName) && head.size() == 1 && head.get(0).isToken()) {
  85:                 TypeRegister result = identifierReference(context, head);
  86:                 context.bindType(head, result);
  87:                 ShiftReduceCallback.super.onReduce(context, production);
  88:                 return;
  89:             }
  90:         }
  91:         TypeRegister result = RULES.resolve(production).build(context, head, production);
  92:         if (result != null) {
  93:             context.bindType(head, result);
  94:         }
```
...
```java
  ...
  35:  * TODO
  36:  *
  37:  * @author <a href="mailto:harvey.blocks@outlook.com">Harvey Blocks</a>
  38:  * @version 1.0
  39:  * @date 2026-04-12 21:47
  40:  */
  41: @Getter
  42: public class ShiftReduceSemanticContext {
> 43:     private static final boolean DEBUG_FUNCTION = Boolean.getBoolean("semantic.debugFunction");
  44: 
  45:     private final ShiftReduceCallbackRegister register;
  46:     @Getter
  47:     private SemanticResult result;
  48:     private final ShiftReducePhaseContext syntaxContext;
  49:     private Iterator<ShiftReduceCallback> callbackIter;
  50:     @Getter
  51:     private final TreeContext treeContext = new TreeContext();
  52:     @Getter
  53:     private final CommandContext commandContext = new CommandContext();
```
...
```java
  257:     public FunctionBodyState currentFunctionBodyState() {
  258:         return functionContext.currentBodyState();
  259:     }
  260: 
  261:     public boolean isCurrentBlockFunctionBody() {
  262:         return !blockFunctionFlags.isEmpty() && blockFunctionFlags.peek();
  263:     }
  264: 
  265:     public void markPendingFunction(FunctionRecord record) {
  266:         if (DEBUG_FUNCTION) {
> 267:             System.out.println("[function-debug] mark pending function: " + record.getSignature().getNameKey());
  268:         }
  269:         pendingFunction = record;
  270:     }
  271: 
  272:     public FunctionRecord consumePendingFunction() {
  273:         FunctionRecord function = pendingFunction;
  274:         pendingFunction = null;
  275:         if (DEBUG_FUNCTION) {
  276:             System.out.println("[function-debug] consume pending function: " + (function == null ? "<none>" : function.getSignature().getNameKey()));
  277:         }
```

> log不会用吗? 还System.out.println
...
```java
  ...
  279:     }
  280: 
  281:     public void scopeIntoBlock() {
  282:         scopeInto();
  283:         FunctionRecord function = consumePendingFunction();
  284:         if (function == null) {
  285:             if (DEBUG_FUNCTION) {
> 286:                 System.out.println("[function-debug] enter ordinary block");
  287:             }
  288:             blockFunctionFlags.push(Boolean.FALSE);
  289:             return;
  290:         }
  291:         enterFunction(function);
  292:         if (DEBUG_FUNCTION) {
  293:             System.out.println("[function-debug] enter function body: " + function.getSignature().getNameKey());
  294:         }
  295:         blockFunctionFlags.push(Boolean.TRUE);
  296:         for (FunctionParameter parameter : function.getParameters()) {
```
...
```java
  13: import org.harvey.vie.theory.syntax.grammar.produce.SimpleGrammarProduction;
  14: 
  ...
  20:  * @author Temper
  21:  */
  22: public class FunctionSemanticCallback implements ShiftReduceCallback {
> 23:     private static final boolean DEBUG_FUNCTION = Boolean.getBoolean("semantic.debugFunction");
  24:     private final ArgumentStepper argumentStepper = new ArgumentStepper();
  25:     private final ParameterStepper parameterStepper = new ParameterStepper();
  26: 
  27:     @Override
  28:     public void onReduce(ShiftReduceSemanticContext context, SimpleGrammarProduction production) {
  29:         onReduce0(context, production);
  30:         ShiftReduceCallback.super.onReduce(context, production);
  31:     }
  32: 
  33:     private void onReduce0(ShiftReduceSemanticContext context, SimpleGrammarProduction production) {
```
...
```java
  ...
  68:                 head
  69:         );
  70:         context.registerFunction(record);
  71:         context.markPendingFunction(record);
  72:     }
  73: 
  74:     private void validateReturnValue(ShiftReduceSemanticContext context, HeadNode head, boolean hasValue) {
  75:         SourceToken returnToken = tokenAt(head, 0);
  76:         if (DEBUG_FUNCTION) {
> 77:             System.out.println("[function-debug] validate return: insideFunction=" + context.insideFunction()
  78:                     + ", currentBlockIsFunctionBody=" + context.isCurrentBlockFunctionBody());
  79:         }
  80:         if (!context.insideFunction()) {
  81:             SemanticDiagnostics.reject(context, returnToken, "return is only allowed inside function body.");
  82:         }
  83:         SemanticType returnType = context.currentFunctionReturnType();
  84:         if (returnType == null) {
  85:             throw new CompilerException("current function return type is missing.");
  86:         }
  87:         if (!hasValue) {
```

> 函数出什么问题不是查清楚了吗? 那就把代码整理一下啊, 没必要的东西清理干净啊
...
```java
  ...
  38:         String defineName = head.getSymbol().isDefine() ? head.getSymbol().toDefine().getName() : "";
  39:         if ("function_head".equals(defineName)) {
> 40:             prepareFunction(context, head);
  41:             return;
  42:         }
  43:         if ("return_stmt".equals(defineName)) {
  44:             validateReturnValue(context, head, head.size() == 3);
  45:             return;
  46:         }
  47:         if ("call_expr".equals(defineName)) {
  48:             validateCall(context, head);
  49:         }
  50:     }
```
...
> 44:             validateReturnValue(context, head, head.size() == 3);
  ...
  50:     }
  51: 
  52:     private void prepareFunction(ShiftReduceSemanticContext context, HeadNode head) {
  53:         SourceToken nameToken = tokenAt(head, 1);
  54:         if (DEBUG_FUNCTION) {
```
...
> 48:             validateCall(context, head);
  ...
  53:         SourceToken nameToken = tokenAt(head, 1);
  54:         if (DEBUG_FUNCTION) {
  55:             System.out.println("[function-debug] prepare function head: " + nameToken);
  56:         }
  57:         if (context.existFunction(nameToken)) {
  58:             SemanticDiagnostics.reject(context, nameToken, "duplicate function declaration is not allowed.");
```
...
### src/main/java/org/harvey/vie/theory/semantic/tag/TagStrategyCompose.java

- **Line**: 111

```java
  101:     public static CommandTranslatorStrategy preciseStringCommand() {
  102:         CommandTranslator doNothing = new DoNotingTranslator();
  103:         CommandTranslator simpleShrink = new SimpleShrinkTranslator();
  104:         CommandTranslator primaryProduceLeftValueTranslator = new PrimaryProduceLeftValueTranslator();
  105:         CommandTranslator arrayAtExpressionTranslator = new ArrayAtExpressionTranslator();
  106:         CommandTranslator memberAccessTranslator = new MemberAccessTranslator();
  107:         CommandTranslator parenthesizedExpressionTranslator = new ParenthesizedExpressionTranslator();
  108:         Map<String, CommandTranslator> exact = new HashMap<>();
  109:         exact.put("program", new ProgramCommandTranslator());
  110:         exact.put("function_head", new FunctionHeadTranslator());
> 111:         exact.put("decl_plain", new DeclarationWithoutInitializationTranslator());
  112:         exact.put("decl_init", new DeclarationWithInitializationTranslator());
  113:         exact.put("return_stmt", new FunctionReturnTranslator());
  114:         exact.put("call_expr", new FunctionCallTranslator());
  115:         exact.put("new_struct_expr", new NewStructTranslator());
  116:         exact.put("new_array_expr", new NewArrayTranslator());
  117:         exact.put("type", new ArrayTypeTranslator());
  118:         exact.put("factor", parenthesizedExpressionTranslator);
  119:         exact.put("assign_stmt", new AssignStatementTranslator());
  120:         exact.put("loc", memberAccessTranslator);
  121:         exact.put("matched_if_stmt", new IfElseStatementTranslator());
```
...
```java
  206:             }
  207:             if (production.containsTag(ProgramSemanticTag.MULTIPLY)) {
  208:                 return new InSuffixExpressionTranslator(operator("multiply"));
  209:             }
  210:             if (production.containsTag(ProgramSemanticTag.DIVIDE)) {
  211:                 return new InSuffixExpressionTranslator(operator("divide"));
  212:             }
  213:             return simpleShrink;
  214:         };
  215:     }
> 216: 
  217:     private static OperatorFactor operator(String name) {
  218:         return new OperatorFactor() {
  219:             @Override
...
```markdown
  25: ### 1. 先确认数组的类型语义
  26: 
  27: 这一阶段首先确认，数组类型不应该把“长度”编码进类型本身，而应该只保留：
  28: 
  29: - 基础元素类型
  30: - 维度层数
  31: 
...
> 35: - `int[][]`
  ...
  42: - `new int[n + 1]`
  43: 
  44: 只是在创建时长度表达式不同，不会形成两种不同类型。
  45: 
```

> 对于100维的数组, 中间代码是怎么设计的? 
...
```java
  6: import org.harvey.vie.theory.lexical.analysis.token.SourceToken;
  7: import org.harvey.vie.theory.semantic.context.ShiftReduceSemanticContext;
  8: import org.harvey.vie.theory.semantic.error.SemanticDiagnostics;
  ...
  11: import org.harvey.vie.theory.semantic.type.SemanticType;
  12: import org.harvey.vie.theory.semantic.type.TypeRegister;
  13: 
  14: import java.util.ArrayDeque;
  15: 
> 16: public final class ArrayCreationDimensions {
  17:     private ArrayCreationDimensions() {
  18:     }
  19: 
  20:     public static Summary summarize(ShiftReduceSyntaxTreeNode node) {
  21:         Summary summary = new Summary();
  22:         visitDimensions(node, dimension -> {
  23:             summary.totalDimensions++;
  24:             if (dimension.containsTag(ProgramSemanticTag.VALUE)) {
  25:                 if (summary.trailingOmittedDimensions > 0) {
  26:                     throw new CompilerException("array creation dimensions with length must precede omitted dimensions.");
```

> 我想问, array的定义 int32[][] a = new int32[1][]; 这样可行吗? 应当是可行的, 因为表示一个int32[]一维数组的锯齿数组, int32[][]的成员是int32[]的数组, 长度不定. 不知道支持不支持
